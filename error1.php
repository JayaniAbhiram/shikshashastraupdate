...
<body>
    ...
    if (isset($_SESSION['validation_errors']) && is_array($_SESSION['validation_errors'])) {
        foreach ($_SESSION['validation_errors'] as $error) {
            if ($error === "A volunteer with the same email or contact already exists") {
                echo '<h3>A volunteer with the same email or contact already exists!<br>Please try again.</h3>';
            } else {
                echo '<h3>' . $error . '</h3>';
            }
        }
